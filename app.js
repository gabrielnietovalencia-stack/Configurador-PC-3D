...
const CONFIG_POR_DEFECTO = {
    ...
    placa:   { escala: 0.234,   pos: { x: 0.44, y: 1.2,   z: -0.67}, rot: { x: 1.57, y: 0.018, z: 1.69 } },
    cpu:     { escala: 0.45,    pos: { x: 0.1,  y: 1.1,   z: -0.5 }, rot: { x: 1.57, y: 0,     z: 0 } },
    ram:     { escala: 0.7,     pos: { x: 0.6,  y: 1.3,   z: -0.6 }, rot: { x: 1.57, y: 0,     z: 0 } },
    grafica: { escala: 0.00282, pos: { x: 0.25, y: 0.88,  z: -0.65}, rot: { x: 3.01, y: -1.55, z: -1.71 } },
};
...
// Un grupo contenedor por tipo — así scene.remove() funciona siempre aunque haya subobjetos
const CONTENEDORES = {};
TIPOS.forEach(t => {
    const contenedor = new THREE.Group();
    contenedor.name = 'contenedor_' + t;
    scene.add(contenedor);
    CONTENEDORES[t] = contenedor;
});

let preciosActivos = { caja:0, placa:0, cpu:0, ram:0, grafica:0 };
const archivoEnCarga = {}; // evita que una carga lenta sobreescriba una más reciente
...
// Limpia el contenedor de un tipo eliminando todos sus hijos
...
window.cambiarComponente = function(tipo, nombreArchivo, nombreBonito, precio) {
    // Token para evitar race conditions entre cargas
    ...
    function colocar(model) {
        // Si llegó una carga más reciente, ignoramos ésta
        if (archivoEnCarga[tipo] !== token) return;

        limpiarContenedor(tipo); // doble limpieza por si acaso
...
window.reiniciarPC = function() {
    TIPOS.forEach(t => {
        archivoEnCarga[t] = null; // cancela cargas en vuelo
        limpiarContenedor(t);
        preciosActivos[t] = 0;
...
// Notificar al HTML que el módulo está listo para recibir clicks
